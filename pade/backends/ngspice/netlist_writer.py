...
class SpiceNetlistWriter(NetlistWriter):
    ...
    def write_netlist(self, cell: Cell, path: Union[str, Path],
                      statements: Optional[list[Statement]] = None) -> None:
    ...
    def generate_netlist(self, cell: Cell,
        ...
        if statements:
            includes = [s for s in statements if isinstance(s, Include)]
            for inc in includes:
    ...
    def _get_subckt_definitions(self, cell: Cell) -> list[Cell]:
        ...
        def collect(c: Cell):
            for subcell in c.get_subcells():
                if subcell.subcells and subcell.cell_name not in seen and not self._is_primitive(subcell):
    ...
    def _format_instance(self, cell: Cell) -> str:
        ...
        nets = ' '.join(t.net.name for t in cell.get_all_terminals(sort=True))
        ...
        if cell_name == 'R':
            ...
            if params:
                return f'{inst_name} {nets} {cell_name} {params}'
            return f'{inst_name} {nets} {cell_name}'
